...
public class JMCh12Exercises
{
    ...
    {
        char[] array = { 'r', 'r', 'r', 'p', 'p', 'p', 'p', 'p', 's', 's', 
            's', 's', 's', 's' };
        
        return array[(int)( Math.random() * array.length )];
    }

    private int mysteryCount( int[] a )
    {
    ...
    public String whatDoesMysteryCountCount()
    {
        ...
        return mCount + " number of zeros before first nonzero";
    }
    ...
    public static int[] makePythagoreanTriple( int m, int n )
    {
        ...
        int [] nums = new int[] { m * m - n *n, 2 * m * n, m * m + n * n };
        return nums;
    ...
    public static int computeScore( String word )
    {
        ...
        {
            int b = alphabet.indexOf(  Character.toLowerCase( a[j]) );
            if ( b >= 0 )
            {
                i += letterScores[b];
            }
    ...
    {
        ArrayList<String> word = new ArrayList<String>( list.size() );
        for (int i = 0; i < list.size(); i++)
        {
            word.add( list.get(list.size() - i - 1) );
        }
        return word;
    }
    ...
    public static boolean covers( double[][] m1, double[][] m2 )
    {
        ...
        return (double)count / larger <= 2.0;
    }
