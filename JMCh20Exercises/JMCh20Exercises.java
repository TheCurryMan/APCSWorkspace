...
public class JMCh20Exercises<E>
{
    ...
    public ListNode<E> reverseList( ListNode<E> head )
    {
        ...
        for (ListNode<E> node = head; node != null; node = node.getNext())
        {
          ListNode<E> newNode = new ListNode<E>(node.getValue(), newHead);
          newHead = newNode;
        }
    ...
    {
        ListNode<String> head2 = null, tail2 = null;
        String sum = "";

        for (ListNode<String> node = head; node != null; node = node.getNext())
        {
          sum += node.getValue();
          ListNode<String> newNode = new ListNode<String>(sum, null);
          if (tail2 == null)
            head2 = newNode;
          else
    ...
    public static void sort( ListNode<String> head )
    {
        ...
        while (firstNode != null)
        {
          ...
          for (ListNode<String> node = firstNode.getNext(); node != null; node = node.getNext())
          {
    ...
    public static ListNode2<Integer> removeMax( ListNode2<Integer> head )
    {
        if (head == null)
          ...
          for (ListNode2<Integer> node = head.getNext(); node != null; node = node.getNext())
            if (((Integer)node.getValue()).compareTo((Integer)maxNode.getValue()) > 0)
              maxNode = node;
          if (maxNode.getPrevious() != null)
            maxNode.getPrevious().setNext(maxNode.getNext());
          else
            head = maxNode.getNext();

          if (maxNode.getNext() != null)
...
/**
 * 1. create a property of type integer to refer to the index of the array 2.
 * retrieve the note 3. make sure each new line assigns the new
 * */
